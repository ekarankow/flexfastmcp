...
"""
from flexfastmcp.server import _collect_x_mcp_overrides, generate_extended_openapi
...
def test_collect_x_mcp_overrides_apply():
    ...
    assert (
        component.parameters["properties"]["limit"]["description"]
        == "Max users to return (1-100)"
    )
    assert "description" not in component.parameters["properties"]["user_id"]


def test_generate_extended_openapi_from_spec_path(tmp_path):
    spec = {
        "openapi": "3.0.0",
    ...
    result = __import__("asyncio").run(
        generate_extended_openapi(spec_path=str(spec_path), as_json=False)
    )
    ...
    assert (
        operation["x-mcp"]["parameters"]["id"]["description"]
        == "Item identifier"
    )
